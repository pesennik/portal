    ...
    public Oauth2CallbackPage(PageParameters pp) {
        ...
        if (!TextUtils.isEmpty(error)) {
            if (error.equals("access_denied")) {
                ...
                setResponsePage(LoginPage.class, LoginPage.errorParams("Во время идентификации произошла ошибка. Пожалуйста, заполните данные вручную."));
            }
            return;
        }
        ...
        try {
            ...
            setResponsePage(LoginPage.class, LoginPage.errorParams("Уникальный ключ не совпадает! Если Вы считаете, что попали сюда по правильной ссылке, пожалуйста, сообщите об этой ошибке на todo@todo"));
        } catch (IOException e) { // happens with VK sometimes
            if (action != OauthLink.Action.LOGIN) {
                HttpUtils.redirectToLastViewedPage(Oauth2CallbackPage.this);
            } else {
                setResponsePage(LoginPage.class, LoginPage.errorParams("Не удалось соединиться с сервером. Пожалуйста, заполните данные вручную."));
            }
        } catch (Exception e) {
